	...
	foreach($eeFields as $eeFieldName => $fieldArray) {  // Field name and settings array
		...
		foreach($fieldArray as $field => $value){ // Checkboxes
			...
			if($field == 'label') { // Text Input
				...
				<td><input type="text" name="eeRSCF_fields[' . esc_attr($eeFieldName) . '][' . esc_attr($field) . ']" value="';
				...
				<td><input type="checkbox" name="eeRSCF_fields[' . esc_attr($eeFieldName) . '][' . esc_attr($field) . ']"';
